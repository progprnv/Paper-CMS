-- Paper-CMS Database Setup for Supabase PostgreSQL
...
-- Create storage bucket policy for papers (run this separately in Supabase Storage)
...
-- IMPORTANT: RLS policies are commented out due to UUID vs INTEGER casting issues
-- Paper-CMS handles authentication and authorization at the application level
-- If you need database-level security, you can implement custom RLS policies
-- that work with your specific authentication setup

-- Uncomment these if you want to enable RLS (not recommended for initial setup)
...
-- Note: Since Paper-CMS uses Flask-Login for authentication (not Supabase Auth),
-- the auth.uid() and auth.jwt() functions won't work as expected.
-- The application handles all security at the Flask application level.

-- If you're using Supabase Auth instead of Flask-Login, you would need to:
-- 1. Modify the Flask app to use Supabase Auth
-- 2. Change user ID to UUID type
-- 3. Implement proper JWT token handling
-- 4. Then enable appropriate RLS policies

-- For now, security is handled by Flask-Login and application-level role checks
